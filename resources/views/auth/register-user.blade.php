...
@section('title', __('Become a User'))

@section('content')
    <p class="auth-eyebrow">{{ __('Join REAP433') }}</p>
    <h1 class="auth-title">{{ __('Become a User') }}</h1>
    <p class="auth-sub">{{ __('Shop the collection and track your orders') }}</p>

    <form class="auth-form" method="POST" action="{{ route('register.user.store') }}" autocomplete="off">
        @csrf
