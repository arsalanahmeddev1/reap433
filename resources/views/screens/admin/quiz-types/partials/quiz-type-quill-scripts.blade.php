@push('styles')
    ...
    <style>
        .quiz-type-quill-wrap .ql-toolbar {
            border: 1px solid rgba(255, 255, 255, 0.12);
            border-bottom: none;
            border-radius: 0.25rem 0.25rem 0 0;
            background: #1a1a1a;
        }
        .quiz-type-quill-wrap .ql-container {
            border: 1px solid rgba(255, 255, 255, 0.12);
        ...
        .quiz-type-quill-wrap .ql-editor {
            min-height: 200px;
        }
        .quiz-type-quill-wrap .ql-editor.ql-blank::before {
            color: rgba(248, 244, 237, 0.45);
        }
        .quiz-type-quill-wrap .ql-editor img {
            max-width: 100%;
            height: auto;
        }
        .quiz-type-quill-wrap .ql-stroke {
            stroke: #f8f4ed;
        }
        .quiz-type-quill-wrap .ql-fill {
            fill: #f8f4ed;
        }
        .quiz-type-quill-wrap .ql-picker-label {
            color: #f8f4ed;
        }
    </style>
@endpush
...
@push('scripts')
    ...
    <script>
        (function() {
            ...
            window.syncQuizTypeQuill = function(quill, textareaId) {
            ...
            window.setQuizTypeQuillContent = function(quill, html) {
                if (!quill) {
                    return;
                }
                if (html && html.trim() !== '') {
            ...
            function uploadAndInsertImage(quill, textareaId, file) {
                ...
                fetch(uploadUrl, {
                    ...
                    .then(function(result) {
                        ...
                        syncQuizTypeQuill(quill, textareaId);
                    })
                    .catch(function() {
                        if (loading && typeof Swal !== 'undefined') {
                            Swal.close();
            ...
            function createQuizTypeQuill(hostSelector, textareaId) {
            ...
            document.addEventListener('DOMContentLoaded', function() {
                window.quizTypeQuillEditors.create = createQuizTypeQuill(
                    '#qt_create_description_editor',
                    'qt-create-description'
                );
                window.quizTypeQuillEditors.edit = createQuizTypeQuill(
                    '#qt_edit_description_editor',
                    'qt-edit-description'
                );
                ...
                document.addEventListener(
                    ...
                    function(e) {
                        ...
                        if (t.id === 'quiz-type-create-form' && window.quizTypeQuillEditors.create) {
                            syncQuizTypeQuill(window.quizTypeQuillEditors.create, 'qt-create-description');
                        }
                        if (t.id === 'quiz-type-edit-form' && window.quizTypeQuillEditors.edit) {
